...
class TestSplayZig(unittest.TestCase):
    def setUp(self):
        self.zig1 = Node("y", Node("x", Node("A"), Node("B"),), Node("C"),)

        self.zig2 = Node("x", Node("A"), Node("y", Node("B"), Node("C"),),)
...
class TestSplayZigZig(unittest.TestCase):
    def setUp(self):
        self.zig_zig1 = Node(
            "z", Node("y", Node("x", Node("A"), Node("B"),), Node("C"),), Node("D"),
        )

        self.zig_zig2 = Node(
            "x", Node("A"), Node("y", Node("B"), Node("z", Node("C"), Node("D"),),),
        )
...
class TestSplayZigZag(unittest.TestCase):
    def test_zig_zag_left(self):
        zig_zag1 = Node(
            "z", Node("y", Node("A"), Node("x", Node("B"), Node("C"),),), Node("D"),
        )

        zig_zag2 = Node(
            "x", Node("y", Node("A"), Node("B"),), Node("z", Node("C"), Node("D"),)
        )
    ...
    def test_zig_zag_right(self):
        zig_zag1 = Node(
            "g", Node("A"), Node("p", Node("x", Node("B"), Node("C"),), Node("D")),
        )

        zig_zag2 = Node(
            "x", Node("g", Node("A"), Node("B"),), Node("p", Node("C"), Node("D"),)
        )
...
class TestSplayGeneral(unittest.TestCase):
        ...
        after = Node(
            ...
            Node(
                ...
                Node(
                    "d",
                    Node("B"),
                    Node("b", Node("c", Node("C"), Node("D"),), Node("E"),),
                ),
            ),
            Node(
                "h",
                Node("g", Node("e", Node("F"), Node("G"),), Node("H"),),
                Node("i", Node("I"), Node("J"),),
            ),
        )
    ...
    def test_fig5a(self):
            ...
            Node(
                ...
                Node(
                    ...
                    Node(
                        ...
                        Node(
                            "c",
                            Node("b", Node("a", Node("A"), Node("B"),), Node("C"),),
                            Node("D"),
                        ),
                        Node("E"),
                    ),
        ...
        after = Node(
            ...
            Node(
                ...
                Node(
                    "d",
                    Node("b", Node("B"), Node("c", Node("C"), Node("D"),),),
                    Node("e", Node("E"), Node("F"),),
                ),
                Node("g", Node("G"), Node("H"),),
            ),
        )
    ...
    def test_fig5b(self):
            ...
            Node(
                ...
                Node(
                    ...
                    Node(
                        ...
                        Node(
                            "c",
                            Node("b", Node("C"), Node("a", Node("D"), Node("E"),),),
                            Node("F"),
                        ),
                    ),
                    Node("G"),
        ...
        after = Node(
            "a",
            Node(
                "f", Node("A"), Node("d", Node("B"), Node("b", Node("C"), Node("D"),),),
            ),
            Node(
                "g", Node("e", Node("c", Node("E"), Node("F"),), Node("G"),), Node("H"),
            ),
        )
